...
import { ArrowLeft, Moon, Sun, LayoutGrid, Link2, ThumbsUp, Users, BadgeCheck, Sparkles, HelpCircle, ChevronDown } from 'lucide-react';
...
const CARDS = [
  ...
  { icon: Users, step: 'The signature', h: 'The per-person number', p: 'Every home shows the real all-in cost split across your group — the figure people actually argue about — recomputed live.' },
  { icon: BadgeCheck, step: 'The decision', h: 'Compare with Scout & lock the pick', p: 'Down to two? Scout weighs price, distance, and your must-haves and calls a winner. The organizer makes one pick official.', gold: true },
];
const FAQ = [
  { q: 'How do I add a home?', a: 'On the board, paste any Airbnb, VRBO, or Booking link into "Add a listing" (an optional price helps if the page hides it). GroupPad scrapes the details, prices it all-in, and drops it into your group\'s submissions to vote on.' },
    ...
    <div className="gp-mobile">
      <div className="mb">
        ...
        <div className="mb-scroll" style={{ paddingBottom: 32 }}>
          <div className="help">
            <div className="help-head"><span className="ey"><Icon icon={Sparkles} className="ico" /> How GroupPad works</span><h1>Pick one rental, together.</h1><p>GroupPad turns "where should we stay?" into one shared board. Here's the whole loop — about a minute to get going.</p></div>
            <div className="help-grid">
              {CARDS.map((c) => (
                <div className={cn('help-card', c.gold && 'gold')} key={c.h}><div className="ic"><Icon icon={c.icon} className="ico" /></div><div className="step">{c.step}</div><h3>{c.h}</h3><p>{c.p}</p></div>
              ))}
            </div>
